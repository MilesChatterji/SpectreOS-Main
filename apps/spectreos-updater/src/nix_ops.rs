...
pub fn run_home_manager() -> Result<(), String> {
    ...
    // Run home-manager through a bash login shell so /etc/profile is sourced
    // and NIX_PATH is set correctly — the same environment hm-setup.sh runs in.
    let cmd = "home-manager switch -b backup --option max-jobs 2 --option cores 2";
